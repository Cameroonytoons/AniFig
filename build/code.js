...
async function applyAnimation(node, animation) {
    ...
    const existingControllers = figma.currentPage.findAll(n => n.getPluginData('targetNode') === node.id);
...
figma.on('selectionchange', () => {
    ...
    nodes.forEach(node => {
        ...
        if (animationName && animationStore[animationName]) {
            applyAnimation(node, animationStore[animationName]);
        }
    });
});
export {};
